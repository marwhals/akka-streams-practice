...
object OpenGraphs extends App {
   ...
   * - emits all the elements form the first source
   * - the all the elements from the second
   */
  ...
  /**
   * Exercise - create a flow that is composed of two other flows
  ...
  val flowGraph = Flow.fromGraph(
    GraphDSL.create() { implicit builder => // builder mutates the shape to become the appropriate static graph component
      import GraphDSL.Implicits._

      /** The operators below only work on shapes */

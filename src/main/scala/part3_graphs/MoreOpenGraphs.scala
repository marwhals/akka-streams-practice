...
object MoreOpenGraphs extends App {
  ...
  val max3StaticGraph = GraphDSL.create() { implicit builder =>
    ...
    // step 4
    UniformFanInShape(max2.out, max1.in0, max1.in1, max2.in1) // Uniform because the receive elements of the same type
...
//  max3RunnableGraph.run()
  ...
  val transactionSource = Source(List(
    Transaction("1234567123", "Paul", "Jim", 100, new Date),
    Transaction("1231231233", "Daniel", "Jim", 100000, new Date),
    Transaction("1231231233", "Jim", "Alive", 7000, new Date)
  ))

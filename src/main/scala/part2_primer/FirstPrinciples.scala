...
object FirstPrinciples extends App {
  ...
  implicit val materializer = ActorMaterializer() // allows for the running of Akka components
...
//  graph.run()
  ...
  val sourceWithFlow: Source[Int, NotUsed] = source.via(flow) //results in a Source type
  val flowWithSink: Sink[Int, NotUsed] = flow.to(sink) // results in a Sink type
...
//  illegalSource.to(Sink.foreach(println)).run()
  // handle NULLs using options

  // various kinds of sources
  val finiteSources = Source.single(1)
  ...
  val headSink = Sink.head[Int] // retrieves head and the closes the stream
...
//  mapSource.runForeach(println) // mapSource.to(Sink.foreach[Int](println)).run()
  ...
  /**
   * Exercise: create a stream that the names of persons, then you will keep the first 2 names with lengths > 5 characters.
   */
  ...
  nameSource.filter(_.length > 5).take(2).runForeach(println) // Short form
